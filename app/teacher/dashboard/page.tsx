  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map((stat, idx) => {
          const topBorders = ["border-t-indigo-500", "border-t-green-500", "border-t-purple-500", "border-t-amber-500"];
          return (
          <div
            key={stat.label}
            className={`bg-white rounded-xl border border-gray-200 border-t-2 ${topBorders[idx]} p-5 hover:shadow-sm transition-shadow`}
          >
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Link
            href="/teacher/images"
            className="flex items-center gap-4 p-5 bg-white rounded-xl border border-gray-200 hover:border-indigo-200 hover:shadow-sm transition-all group"
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900">Manage Images</p>
              <p className="text-xs text-gray-500 mt-0.5">Upload, review, and organize practice images</p>
            </div>
            <svg className="w-4 h-4 text-gray-300 shrink-0 group-hover:text-indigo-500 group-hover:translate-x-0.5 transition-all" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
            </svg>
          </Link>
          <Link
            href="/teacher/class"
            className="flex items-center gap-4 p-5 bg-white rounded-xl border border-gray-200 hover:border-green-200 hover:shadow-sm transition-all group"
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900">View Class</p>
              <p className="text-xs text-gray-500 mt-0.5">See student enrollment and class details</p>
            </div>
            <svg className="w-4 h-4 text-gray-300 shrink-0 group-hover:text-green-500 group-hover:translate-x-0.5 transition-all" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
            </svg>
          </Link>
        </div>
          ...
          {recentSessions.length === 0 ? (
            ...
            <div className="divide-y divide-gray-50">
              {recentSessions.map((s) => {
                ...
                const scoreColorClass = total >= 20 ? "text-green-600" : total >= 12 ? "text-yellow-600" : "text-red-500";
                const dotColorClass = total >= 20 ? "bg-green-500" : total >= 12 ? "bg-yellow-500" : "bg-red-400";
                return (
                  <Link
                    key={s.id}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full shrink-0 ${dotColorClass}`} />
                      <span className="text-sm font-medium text-gray-900 group-hover:text-indigo-700 transition-colors">{s.user.name}</span>
                      <span className={`px-2 py-0.5 text-xs rounded-full ${themeBadgeColor(s.image.theme)}`}>
                        {themeLabel(s.image.theme)}
                      </span>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={`text-sm font-semibold ${scoreColorClass}`}>{total}/30</span>
                      <span className="text-xs text-gray-400">
                        {s.completedAt ? new Date(s.completedAt).toLocaleDateString() : ""}
                      </span>
                      <svg className="w-4 h-4 text-gray-300 group-hover:text-indigo-500 group-hover:translate-x-0.5 transition-all" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                      </svg>
                    </div>
                  </Link>
